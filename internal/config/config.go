...
//
// Валидация здесь не выполняется намеренно: такой конфиг заведомо её
// не пройдёт, в том и смысл.
...
// библиотеки будет обработана дважды. Молча это не разрулить — только
// требованием раздельных деревьев.
...
// с пустым адресом — это опечатка в конфиге, и молчать о ней нельзя: иначе
// пользователь будет неделями думать, что библиотека обновляется.
...
func (c *Config) Validate() error {
	...
	// Требуем все три набора ключей сразу, осознанно: иначе пришлось бы
	// городить fallback-ветки на каждое сочетание отсутствующих провайдеров.
	var missing []string
